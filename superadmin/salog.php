...
$sa_email = filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL); 
$sa_password = filter_input(INPUT_POST, 'password', FILTER_SANITIZE_STRING);




// Retrieve details from database for the given email
$whereCondition = "sa_email = ?";

// // Prepare the SQL query with the WHERE clause and bind the variable to a parameter
$stmt = $conn->prepare("SELECT * FROM supadmin WHERE $whereCondition");
...
if ($stmt->execute()) {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        if (password_verify($sa_password, $row['sa_password'])) {
            ...
            try {
                ...
                $mail->addAddress($sa_email, $row['sa_name']);     //Add a recipient
                

                //Content
                $mail->isHTML(true);                                  //Set email format to HTML
                $mail->Subject = 'Here is the subject';
                $mail->Body = '<p>Hello '.htmlspecialchars($row['sa_name']).'</p><p>Your One Time Password is: </p><h1>'.$otp.'</h1>';

                $mail->send();
                
                // Redirect user to authentication page after sending OTP email
                session_start();
                $_SESSION['auth_id'] = $row['sa_id'];
                $_SESSION['auth_email'] = $row['sa_email'];
                $_SESSION['auth_name'] = $row['sa_name'];
